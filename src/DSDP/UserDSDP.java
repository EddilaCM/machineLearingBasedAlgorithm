...
public class UserDSDP extends Cluster {
	 ...
	 *************** 
	 */
	public UserDSDP() throws IOException, Exception {
		super();
	...
	public void randomSelectCenters() {
		...
		for (int i = 0; i < k; i++) {
			for (int j = 0; j < ClusterDataInfo.CHAN_LEN; j++) {
				centers[i][j] = ClusterDataInfo.uChannels[tempIndices[i]][j];
			} // Of for j
		} // Of for i
	...
	public void clusterUsingCenters() {
		for (int i = 0; i < ClusterDataInfo.userNum; i++) {
			...
			for (int j = 0; j < centers.length; j++) {
				double tempUserDist = KLDivergenceDistance(ClusterDataInfo.uChannels[i], centers[j]);
//				System.out.println("tempUserDist: " + tempUserDist);
	...
	public void meanAsCenters() {
		...
		for (int i = 0; i < ClusterDataInfo.userNum; i++) {
			...
			for (int j = 0; j < ClusterDataInfo.CHAN_LEN; j++) {
				centers[blockInformation[i]][j] += ClusterDataInfo.uChannels[i][j];
			} // Of for j
		} // Of for i
	...
	public static void test() throws Exception {
		...
		UserDSDP tempKMeans = new UserDSDP();

		System.out.println(tempKMeans);

		int tempGroupNum = 3;
		tempKMeans.cluster(tempGroupNum, 0.05);
		System.out.println(Arrays.toString(tempKMeans.blockInformation));
		System.out.println(Arrays.toString(tempKMeans.blockSizes));
		OutputGroupFile.outToGrpFile(userGroupPath, tempGroupNum, tempKMeans.blockInformation);
